...
- Le consulter
*** TODO SmartSense: Corriger le formatage des entrées pour InfluxDB
...
- Corriger la gestion des chaînes de caractères
*** TODO SmartSense: Créer issue au sujet de la dépréciation de telnetlib
- Librairie dépréciée
...
- Métriques à observer
  ...
  - Débit en écriture
  - Volume des valeurs
**** IN-PROGRESS SmartSense: Configurer inch pour simuler le plus fidèlement le workload du RPi
...
**** IN-PROGRESS SmartSense: Créer une image QEMU RPi 32-bits
...
- SmartSense: Configurer inch pour simuler le plus fidèlement le workload du RPi
  ...
  - Aurai tout de même un débit en terme de points supérieur à la workload réelle
  ...
  - Observations
    ...
    - Volume : 80Mo/shard
  - À tester
    - ~/go/bin/inch -v -db SensorData -delay 200ms -m 5 -t 1,2,1,2,1,1,1,1 -f 420 -p 86400 -b 10 -consistency any
...
- SmartSense: Créer une image QEMU RPi 32-bits
  ...
  - [ ] Activer le SSH
  - [ ] Convertir l'image OS en image qcow2
  - [ ] Redimensionner l'image
  - [ ] Redimensionner la partition
  - [ ] MàJ l'OS
  - [ ] Créer snapshot base-image
  - [ ] Installer InfluxDB
  - [ ] Installer golang
  - [ ] Installer inch
  - [ ] Mettre en place la config d'InfluxDB
  - [ ] Désactiver le redémarrage automatique d'InfluxDB
  - [ ] Créer la base de données de test avec la bonne retention policy
  - [ ] Créer snapshot influxdb-inch
...
- SmartSense: Monitoring d'InfluxDB OS 64-bits, workload par inch
  - Commande
    - ~/go/bin/inch -v -db SensorData -delay 1s -m 5 -t 1,2,1,2,1,1,1,1 -f 84 -p 86400 -b 50 -consistency any
...
- SmartSense: Reproduire l'environnement sur lla VM
